...
def process_single_frame(frame: np.ndarray, coin_name: str, calibrator: calibration.Calibrator) -> tuple[np.ndarray, list[dict[str, Any]]]:
    """单帧定义"""
    ...
    for cnt in contours:
        ...
        diameter_mm = calibrator.pixels_to_mm(features['diameter_pixels'])
...
def run_batch(input_path: str, output_path: str, coin_name: str, calibrator: calibration.Calibrator) -> list[list[dict[str,Any]]]:
    ...
    for i, img in enumerate(imgs):
        ...
        result_img, all_features = process_single_frame(img, coin_name, calibrator)
